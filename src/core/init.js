...
export async function initializeProject(projectDir, options = {}) {
  ...
  const project = entity("project", name, { title, format: "16:9", source_ids: [source.id], outline_id: outline.id, theme_id: theme.id, asset_ids: [], ...(options.deliveryMode ? { delivery_mode: options.deliveryMode } : {}), ...(options.accessibilityProfile ? { accessibility_profile: structuredClone(options.accessibilityProfile) } : {}) });
