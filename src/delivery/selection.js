...
export async function recordDeliverySelection(projectRoot, input, options = {}) {
  ...
  const formats = [...new Set(input.formats.map((value) => String(value).toLowerCase()))];
...
export async function readDeliverySelection(projectRoot, id) {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(id ?? "")) throw new DeliverySelectionError("DELIVERY_SELECTION_INVALID", "delivery selection id is invalid");
...
export async function exportOutlineMarkdown(project, selection) {
  if (selection.artifact_type !== "outline" || !selection.formats.includes("markdown")) return [];
  const outline = project.contracts.outline;
  const pages = new Map(project.contracts.pages.map((page) => [page.id, page]));
  const lines = [`# ${project.project.title} — Approved outline`, ""];
  for (const section of outline.sections) {
    lines.push(`## ${section.title}`, "");
    for (const pageId of section.page_ids) {
      const page = pages.get(pageId);
      if (page) lines.push(`- ${page.screen_text?.title ?? page.three_second_message} — ${page.task}`);
    }
    lines.push("");
  }
  const relative = path.join(".pptops", "delivery-selections", selection.id, "outline.md");
  const file = resolveProjectPath(project.root, relative);
  await fs.writeFile(file, `${lines.join("\n").trim()}\n`, { flag: "wx" });
  return [{ format: "markdown", name: "outline.md", path: file }];
}
