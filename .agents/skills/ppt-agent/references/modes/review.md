...
Report the visual-asset inventory, classification counts, generated-asset evidence findings, automated checks, rendered inspection, Agent or human visual observation, explicit user acceptance, real Microsoft PowerPoint acceptance, and business acceptance separately. When `delivery_mode` is declared, report its automated fit separately, using the mode's visible-copy guidance and any declared workshop interaction. Do not count speaker-note intent or interaction metadata as visible slide text, and do not present timing estimates as rehearsal evidence. Missing evidence stays pending. Exit with an immutable Review record.
