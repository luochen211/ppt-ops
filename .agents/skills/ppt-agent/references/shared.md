...
When the user requests a speaker script, store reviewed authored text in `speaker_notes`; use `speaker_note_intent` only for its planning purpose. HTML offers an independent Speaker notes link, and PPTX uses native notes. Never copy the script into `screen_text` or treat an unrehearsed time estimate as verified timing.
