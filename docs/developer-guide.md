...
`source-update-preview` imports a changed Markdown, DOCX, or PPTX file as a separate immutable Source snapshot and reports page-level impacts from existing `source_refs`. Missing or unrecognized locators are marked `manual_review`. `source-update-propose` can create a Candidate for one impacted page using an explicit content patch and the current page revision; the normal preview, PowerPoint observation, user decision, and stale-base checks still apply. These commands do not update `source_refs` or assert that the new source is factually correct. Exact field/claim impact, source rebinding, audience-variant decisions, and delivered-artifact warnings depend on the fact ledger in #90 and require later work for #98.
